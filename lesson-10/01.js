...
const model = {
  ...
  deleteMovie(targetID) {
    this.movies = this.movies.filter((m) => m.id !== targetID)
    view.init()
  }
  // your code
}
...
const view = {
  init() {
    ...
    list.addEventListener('click', (elem) => {
      if(elem.target.className === 'delete-button') controller.deleteMovie(+elem.target.parentElement.id)
    })
    // your code
  },
  renderMovies(movies) {
